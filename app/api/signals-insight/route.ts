...
export async function POST(request: NextRequest) {
  ...
  // "내부 거래"를 그냥 쓰면 모델이 계열사 간 특수관계자 거래로 읽는다. 실제로
  // "승인 절차 준수", "보고 체계의 투명성" 같은 엉뚱한 권고가 나왔다. 이 자료가
  // 무엇인지 먼저 못박는다.
  const prompt = `당신은 중소기업의 거래 실적을 읽는 애널리스트입니다.

아래 수치는 이 기업이 제출한 거래명세서·세금계산서에서 읽어낸 이 회사와 거래처
사이의 매출 거래 실적입니다. 계열사 간 내부거래(특수관계자 거래)가 아닙니다.
따라서 내부통제·승인 절차·공시 의무 같은 이야기는 쓰지 마세요.

지표와 판정은 이미 정해진 계산식으로 산출된 것입니다. 새로 판단하거나 숫자를
만들지 말고, 주어진 판정을 그대로 전제로 설명만 작성하세요.

분석에 사용한 거래: ${transactionCount}건
종합 판정: ${
    dataSufficient
      ? `긍정 ${positiveCount}개 / 주의 ${cautionCount}개 → 거래 활동 ${activityLevel}`
      : "표본이 부족해 판정하지 않음"
  }
...
- 1문장: 주의 판정을 받은 지표가 있으면 그것을, 없으면 가장 뚜렷한 긍정 지표를 골라
  그 수치가 이 기업에 무엇을 뜻하는지 쓰세요. 어떤 지표인지 이름과 수치를 밝히세요.
  "지표가 하나 있으며"처럼 뭉뚱그리지 마세요.
