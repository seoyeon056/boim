...
async function main() {
  ...
  if (!key) {
    await writeFile(OUT, "export const DART_CORPS: string = \"\";\n", "utf8");
  ...
  await writeFile(OUT, `export const DART_CORPS: string = ${JSON.stringify(text)};\n`, "utf8");
...
main().catch((error) => {
  ...
  return writeFile(OUT, "export const DART_CORPS: string = \"\";\n", "utf8");
});
